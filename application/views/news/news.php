...
<?php foreach($this->m_general->getNewsTopsList()->result() as $listTop) { ?>
<div class="GalleryItem GalleryItem--mediumMargins GalleryItem--adaptiveMargins GalleryItem--landing is-focus">
  <a href="<?= base_url(); ?>news/post/<?= $listTop->id_new ?>" data-external="false" data-article-id="<?= $listTop->id_new ?>" data-analytics="news-promotion" data-analytics-placement="blog:<?= $listTop->id_new ?> - <?= $this->m_general->getNewTitle($listTop->id_new); ?>" class="ArticleLink FeaturedArticle-link align-left">
  ...
  <div style="background-image:url(<?= base_url(); ?>assets/images/news/<?= $this->m_general->getNewImage($listTop->id_new); ?>)" class="Card-image">
</div>
</div>
<div class="Card-content">
<div class="FeaturedArticle-text text-base multiline max-lines-3 flush-top position-relative text-heavy text-upper">
  <div class="TextOverflow text-truncate-ellipsis" data-original-text="<?= $this->m_general->getNewTitle($listTop->id_new); ?>"><?= $this->m_general->getNewTitle($listTop->id_new); ?></div>
</div>
</div>
</div>
</a>
...
<?php foreach($this->m_general->getNewsList()->result() as $list) { ?>
...
<span class="ArticleCommentCount-count"><?= $this->m_general->getCommentCount($list->id)->num_rows(); ?></span>
</a>
